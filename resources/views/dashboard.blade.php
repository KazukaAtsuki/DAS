...
<style>
    :root {
        ...
        --das-gray: #64748b;
        --das-bg-soft: #f1f5f9;
        --rca-color: #ef4444; /* Warna Merah RCA */
    }
    ...
    /* --- STYLE KHUSUS MODE RCA (AUDIT MODE) --- */
    .dashboard-container.mode-rca-active {
        background-color: #fff1f2; /* Merah muda sangat tipis */
        border: 2px solid var(--rca-color);
        border-radius: 20px;
        padding: 20px;
        margin-top: 100px;
    }

    /* Ubah warna header dashboard saat RCA */
    .mode-rca-active .dashboard-title {
        color: var(--rca-color) !important;
    }

    .mode-rca-active .card-sensor {
        border-color: var(--rca-color);
        box-shadow: 0 10px 30px rgba(239, 68, 68, 0.1);
    }

    .mode-rca-active .card-sensor::after {
        background: var(--rca-color); /* Ganti garis atas card jadi merah */
    }
...
@section('content')
<!-- Tambahkan meta token untuk AJAX POST -->
<meta name="csrf-token" content="{{ csrf_token() }}">

<!-- Tambahkan ID agar bisa dimanipulasi JS dan class conditional dari PHP -->
<div id="dashboard-wrapper" class="container-fluid dashboard-container {{ $isRcaOn ? 'mode-rca-active' : '' }}">
    ...
    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between mb-5 gap-3">
        ...
        <!-- Title & Breadcrumb -->
        <div>
            <h2 class="fw-bolder text-dark mb-1 dashboard-title" style="letter-spacing: -0.5px;">
                {{ $isRcaOn ? 'RCA / AUDIT MODE ACTIVE' : 'Monitoring Dashboard' }}
            </h2>
...
@push('scripts')
<script>
    // 1. Setup CSRF Token untuk keamanan AJAX
    $.ajaxSetup({
        headers: {
    ...
    $(document).ready(function() {
        // Ambil stack_id dari URL
        const urlParams = new URLSearchParams(window.location.search);
        const stackId = urlParams.get('stack_id') || "";

        // Fungsi Auto Refresh Data Sensor (Setiap 2 Detik)
    ...
    // 2. KONFIGURASI NOTIFIKASI MODERN (TOAST)
    ...
    function toggleRcaMode() {
        // Matikan tombol & tampilkan loading spinner
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.is_rca_mode) {
                    ...
                    // Ubah Tampilan Dashboard jadi Merah
                    $('#dashboard-wrapper').addClass('mode-rca-active');
                    $('.dashboard-title').text('RCA / AUDIT MODE ACTIVE');

                    // Tampilkan Notifikasi Warning Modern
                    Toast.fire({
                        icon: 'warning',
                        title: 'RCA Mode Activated',
                        text: 'System is now recording Audit Data.'
                    });

                } else {
                    // --- JIKA MODE NORMAL (HITAM) ---
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: 'RCA Stopped',
                        text: 'System returned to Normal Mode.'
                    });
                }
            },
            error: function(xhr) {
                ...
                Toast.fire({
                    icon: 'error',
                    title: 'Action Failed',
                    text: 'Could not change system mode. Try again.'
                });
            },
            complete: function() {
                // Nyalakan tombol kembali & sembunyikan loading
